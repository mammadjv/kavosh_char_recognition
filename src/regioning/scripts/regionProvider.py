...
#                        if(x < width/5 or x > 4*width/5 or x + w < width/5 or x+w > 4*width/5):
                        if(x> 4*width/5 or y > 4*height/5):
                        #       print "1"
                        	continue
...
#		thresh = cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY_INV,31,2)
		valid_contour = self.checkFirstFrame(thresh)
		if(valid_contour == True):
			print 'valid!!!!!'
		else : 
			print 'not valid'
			return image,image,image,False,image,thresh
		_, contours, hierarchy = cv2.findContours(thresh , cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
		best_x , best_y , best_w , best_h = 0 , 0 , 0 , 0
		#print image.shape
		draw_image = image.copy()
		height, width = image.shape[:2]
		thresh_draw = cv2.cvtColor(thresh, cv2.COLOR_GRAY2BGR)
		for cnt in contours :
			x,y,w,h = cv2.boundingRect(cnt)
			cv2.rectangle(thresh_draw,(x,y),(x+w,y+h),(255,255,0),2)
#			if(w  > 4 * width /5):
#				continue
#			if(x < 10 or x > width -10 or x + w < 10 or x+w > width -10):
			if(h < height/2 and y < height/3 and y+h < height/3):
				continue
			if(x > 4*width/5 or y > 4*height/5):
			#	print "1"
				continue
...
#				continue
			if(w*h > best_w*best_h):
				best_x , best_y , best_w , best_h = x , y , w , h
			cv2.rectangle(thresh_draw,(x,y),(x+w,y+h),(255,0,0),2)
		edged = thresh 
		crop_img = image
		contour_found = False
		if(best_w > 15 and best_h > 15):
			...
			contour_found = True
		return image ,thresh_draw ,crop_img , contour_found , edged, thresh
